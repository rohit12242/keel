...
| `agents/reviewer.md` | Read-only. Reviews the finished branch against the brief's Output column, the contract, the NFR questions, the architecture rule, scope and tests. Produces the handover report — the same shape as the W3-12…16 handover, minus the pre-written review-gate answer. It opens with `Do next`: at most three things to change with their `file:line`, and the finding counts, so the size of the review is visible before the detail. | NFR-03/07/09/10/11/12, ADR-001, ADR-007 |
